defmodule ElixirGoogleForms.FormsTest do
  ...
  describe "forms" do
    ...
    test "create_form/1 with valid data creates a form" do
      valid_attrs = %{active_until: ~U[2021-11-09 06:06:00Z], name: "some name"}

      assert {:ok, %Form{} = form} = Forms.create_form(valid_attrs)
      assert form.active_until == ~U[2021-11-09 06:06:00Z]
    ...
    test "update_form/2 with valid data updates the form" do
      form = form_fixture()
      update_attrs = %{active_until: ~U[2021-11-10 06:06:00Z], name: "some updated name"}

      assert {:ok, %Form{} = form} = Forms.update_form(form, update_attrs)
      assert form.active_until == ~U[2021-11-10 06:06:00Z]
